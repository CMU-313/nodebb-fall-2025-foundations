...
define('admin/manage/categories', [
	...
	// Document-level purge handler to cover dropdown clicks and cases where
	// rows are rendered dynamically or Categories.init wasn't called.
	(function bindAdminPurgeHandlers() {
		const selector = '[data-action="purge"][data-cid]';
		$(document).off('click.adminPurge', selector).on('click.adminPurge', selector, function (e) {
			e.preventDefault();
			const $btn = $(this);
			...
			if (!cid) {
				console.error('admin/manage/categories: purge clicked but no cid', this);
				return;
			}
